...
26 engine-verified player stories mapping the readiness state space. Every row was **hand-computed from `READINESS_TUNING` first, then verified against the real engine twice** (by its designing agent and by an independent re-run) — zero discrepancies, so the tables double as verification that the calculation is right. Each row is pinned as a regression test in `test/readinessScenarios.test.ts`.
...
|---|---|---|---|---|---|---|---|---|
| `weekend-warrior` | 5 games Sat+Sun, 4 weekends | 0 | −3 | 0 | 0 | **72** | fresh | Every overload arm silent; only the low-frequency nudge (2 active days/wk < 3) costs −3. Confidence **low** — no mental logs, no stats. |
...
| `weekend-only-consistency-nudge` | Weekends only, always calm | 0 | −3 | 0 | 0 | **72** | fresh | The undertraining side below the layoff threshold: nothing wrong, just "2 days/week is thin stimulus to improve." |
...
| `deaths-improve-damage-fall` | Damage −30%, deaths improve 5→4 per 10 | 1 | 0 | 0 | 0 | **75** | fresh | The sign-alignment pin: fewer deaths counts FOR you and cancels the damage drop inside the blended game score — a style change, not a slump. |
