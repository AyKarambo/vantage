...
| [`readiness/`](../../src/core/readiness) | Training-load / supercompensation model: gap-based sessions, local 4am-day buckets, EWMA acute-vs-chronic load, per-account/per-hero stat baselines with a CUSUM decline detector, a disagreement-gated subjective read, the target-focus dampener, and the score-first composite (band derived from score + driver + hard gates). Conservative, centrally-tuned constants. |
...
(assets) · `certum-sign.cjs` (build sign hook),
