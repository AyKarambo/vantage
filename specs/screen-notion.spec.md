...
- **Status card:** "Connected to Notion" with the database title (when known) and ready-to-sync count, or "Not connected" with the precise reason (no token · token saved but no database selected · using the `appsettings.json` fallback · not ready). A distinct **"Database shape mismatch"** state when connected but the configured database's properties don't match the Gametracker schema, listing the missing/mismatched property names. A **Schema** section notes any columns Vantage auto-added this cycle, or a provisioning error. The connected state also carries an **"Open in Notion ↗"** link (`NotionStatus.gametrackerUrl` via `bridge.openExternal`, W6) — the same target the tray's "Open Gametracker in Notion" item opens, previously the only place to reach it.
...
- **Sync now card:** the button is enabled only when connected **and** there are unsynced games, labelled with the **unsynced (needs-sync) competitive game count** (`NotionStatus.unsyncedGames`) — not the total history. Result chips report N synced / N skipped / N failed; every state has a contextual note. A shape mismatch short-circuits the sync with an inline error instead of failing every row.
  ...
  - **Failures list + retry (W6):** when a sync's `ExportResult.failures` carries one or more `{matchId, map, reason}` entries, they render as a collapsible list under the result chips with a **"Retry N failed"** button that re-runs export scoped to exactly those match ids via `exportNotionMatches` (bypassing the active filter bar entirely, so a retry can never silently miss a game the filters happen to exclude). Shares the same progress/result rendering as the main Sync button.
  - **Last synced (persisted, W6):** a **`NotionStatus.lastSyncResult`** line — `Last synced <relative time> — N synced · M updated · K failed (<reason>)` — read from `config.local.json`'s `notion.lastSyncResult`, stamped by `NotionRuntime.export()` on every REAL attempt (including one where nothing landed), not only a successful one. This is deliberately distinct from `lastSyncedAt`, which only ever moves when something actually synced (`ok > 0 || updated > 0 || skipped > 0`) — before this, a fully-failed run left the timestamp frozen with no way to tell the last attempt had failed. The in-card transient result (the failures list above) is this visit's answer; `lastSyncResult` is what the NEXT visit or an app restart sees. The sync/retry actions deliberately do not force-refresh `NotionStatus` afterward — that would rebuild this whole card and wipe the transient result it just rendered.
