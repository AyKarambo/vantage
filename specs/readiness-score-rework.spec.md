...
## Amendment (2026-09-03) — cross-account hero experience & main-account weighting

Owner request after a false *"still learning Shion, Genji"* note: Genji has 244 lifetime games pooled across the player's accounts but only 6 on the SirTilt alt, and the learning-window exemption was keyed per (account, hero). Two rules change; everything else in this spec (and in `readiness-data-regimes.spec.md`) stands.

- **(a) Hero experience is a property of the player, not the account.** The learning-window count (`heroLearnGames`) now pools lifetime hero games **across all accounts** (`Baselines.heroLifetime` is keyed by hero alone). Stat baselines — the per-hero / per-role buckets — stay strictly **per account** (Resolved #3 unchanged); only the "have I learned this hero" count pools. A hero with ≥ 12 games on the main is never "still learning" on an alt.
- **(b) Main-account damper.** The account with the most games among those played inside the chronic window is the **main** account (tie-break: most recent game, then lexical; the `Unknown` bucket never competes). The objective subscore is computed **account-blind** — every detector, gate, coverage sum and the blend see one game as one game — and the finished delta is then multiplied by `1 − altShare × (1 − altAccountWeight)`, where `altShare` is the share of the acute window played away from the main. An all-alt week therefore keeps **0.35** of whatever the read would have been; a single-account window keeps all of it and is bit-identical to the previous engine (pinned by golden tests). The damper is applied to the reward and the punishment alike, so an alt week can neither cost nor earn what the same week on the main would. Deliberately NOT a per-game weight inside the accumulators: a weight inside a one-sided CUSUM scales its recovery steps too, so playing *well* on an alt between two poor main games would push the accumulator up and fire a decline the unweighted engine never saw; weights inside the coverage sums likewise drag the regime blend toward `manual` and switch on the promoted caps this exists to avoid. The `mixed-accounts` note names the main account when a damper is actually in effect; a window with alt games below the mixed bar surfaces a quieter `alt-weighted` note instead. Neither note is raised by unresolved (`Unknown`) captures.
- **Landed alongside:** the per-10 divisor is now **played time** (`src/core/playedTime.ts` — measured from round events on newer captures, estimated from the wall clock on older ones, taken as typed on manual logs); `minPer10Minutes` gates on it.

## Amendment (2026-09-04) — rank-aware main-account damper

Owner feedback on the (b) damper above: a flat `altAccountWeight` treated every alt account the same regardless of how close its rank actually was to the player's own level — "if I play on an alt account which is close to the rank of my main account ... then it can be fully taken into account". The damper is now rank-proximity-based rather than a flat per-account fraction; (a) and the account-blind detector design from the 2026-09-03 amendment are unchanged.

- **The weight is per game, not per account.** For an alt-account game, `altGameWeight` (`performance.ts`) compares that game's rank (`GameRecord.rankAtStart`, when logged) against the main account's **usual** rank — the MEDIAN rank scalar (`rankToPoints`, `../rank/scalar`) held going into the main's own games over a trailing `mainRankWindowDays` (**90**) window, not today's number alone, so one hot or cold streak can't move what counts as "close." Only a gap **below** the main's usual rank costs anything — an alt played at or above that level is never penalised, whatever the reason for the second account.
- **Three bands, linear between.** Within `altRankCloseGap` (**2.5** divisions, 100 rank-scalar points each) below the main's usual rank, the game counts in full (weight 1) — the case the owner named directly: "mid/high master" reading as the same level as "low gm". At/beyond `altRankSmurfGap` (**7.5** divisions below) the weight bottoms out at the floor, `altRankFloorWeight` (**0.15**) — "dia/emerald is basically smurfing at this point". Between the two thresholds the weight tapers linearly, so "low master" (a middling gap) lands roughly halfway, not at either extreme.
- **`altDamp` is the mean of the per-game weights** over the acute window (main-account games always contribute 1) and replaces the old `1 − altShare × (1 − altAccountWeight)` formula as the multiplier on the finished delta — reward and punishment alike, as before. `altAccountWeight` (**0.35**) is now the FALLBACK used only when a rank genuinely can't be compared (no anchor on one side or the other, and no earlier game on that account to fall back to either) — the same number the flat design used everywhere, so missing data degrades to the prior behaviour rather than to no damping at all. A single-account history, or a history where every alt game reads as rank-close, still has `altDamp = 1` and is bit-identical to the account-blind engine.
- **`PerfState` gains `altDamp: number`** (the multiplier actually applied; `altShare` is unchanged — the RAW, unweighted share of the window on a different account, still what gates the `mixed-accounts` / `alt-weighted` notes).
- **The crown needs a clear lead again.** The 2026-09-03 amendment picked the main by plain plurality (no margin), reasoning that a flat per-account weight made a near-tie harmless — whichever account was crowned, the OTHER got the same `altAccountWeight`. That symmetry breaks under rank-proximity weighting: the main doesn't just decide who is damped, it supplies the RANK the other account is measured against, and a higher-ranked account always reads as "at or above main" regardless of which one wears the crown. `mainAccountLeadMargin` (**10%** more games than the runner-up) is restored so an ordinary near-even split between two differently-ranked accounts returns **no main** (`altDamp = 1`, nobody damped) rather than flip-flopping between two damped states on a single game. A count tied exactly at the top always fails the margin regardless of order, so `mainAccountOf` no longer needs (or has) a recency/lexical tie-break.
