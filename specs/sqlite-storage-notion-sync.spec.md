...
- **Settings → Data storage** shows the current folder and offers **Change…**, migrating **all** data files (`history.db`, `manual.json`, `outbox.json`, `rankAnchors.json`, `placements.json`, `checkIns.json` — S10 phase 2's pre-session check-ins, `screen-mental.spec.md` — and the frozen legacy `history.json` when present) with a copy-verify-then-delete guarantee: originals are removed only after the switch is committed; failures leave the old location fully intact; missing optional files are skipped. A target that already contains Vantage data offers **adopt or cancel** — never overwrite.
