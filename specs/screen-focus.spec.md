...
**Source:** `renderer/src/views/focus.ts`, `src/core/analytics/focus.ts`, `src/core/masterData/resolver.ts` (`makeMapActive`).
...
- **Three sections — Roles, Heroes, Maps** (H1), each rendered from `DashboardData.focusItems` (`focusEntries` + `linkFocusTargets`, `src/core/analytics/focus.ts`, unit-tested in `test/focus.test.ts`), filtered by `dimension`. A section with no net-losing entries is omitted entirely rather than shown empty; when ALL THREE are empty, a single positive empty state replaces them.
  ...
  - **Since-flagged progress** [explicit]: an entry linked to an active, non-archived authored target (name mentions the entry's key — both sides normalized to lowercase alphanumerics, so casing/apostrophe styles all link; a role also matches its camelCase-split display form, e.g. "Open Q"; the Notion bookkeeping pseudo-target never links; the most recently flagged match wins) shows a progress line instead of the quick-create button: "▴ 4 pts since you flagged it (<date>) · N games since", or "◎ tracking "<target>" since <date> · N games since" while either window still has < 3 decided games. Progress is computed over the **unfiltered** competitive history (the target's lifetime, like staleness), with `deltaPts` = (winrate since − winrate before) × 100. A target whose name mentions no entry key simply never links — no error.
  - Unlinked, in-pool rows keep the **＋ target** quick-create → navigates to Targets with prefill "Practice <name>: warm up unranked + review one replay" and, for a hero or role entry (H1), that hero/role pre-selected in the builder's Scope picker too (`ViewParams.prefillHeroes` / `.prefillRole`) — a map entry only pre-fills the name, since there's no map scope on a target.
...
- Given a map the master-data catalog marks inactive, then its row carries an "Out of pool" tag, sorts behind every in-pool map row regardless of score, and has no "＋ target" button.
- Given an active target whose name mentions an entry's key, then that row shows the since-flagged progress line (delta once both windows have ≥ 3 decided games) and no quick-create button; a target mentioning no entry key never links (no error).
- Given a click on "＋ target" on an unlinked, in-pool row, then the app navigates to Targets with the practice-framed name prefilled, plus the matching role/hero scope pre-selected for a hero or role entry.
