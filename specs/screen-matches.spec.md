...
- **Edit tracking:** the header carries an **Edit tracking** ("Add tracking" when ungraded) action opening a modal for the match's full **manual layer**. For hand-logged matches the game facts (result/role/map/hero/mode) are editable; for auto-tracked (GEP) matches those stay **locked** and only the manual layer changes — mental flags (incl. leaver-team), the per-match SR %, and the active targets' grades. Saves go through `editMatch`; `ctx.refresh()` re-pulls the detail so dependent views update.
