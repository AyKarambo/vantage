...
export function horizontalBars(data: WrPoint[], opts: { compact?: boolean } = {}): HTMLElement {
  ...
  wrap.append(
    ...data.map((d) => {
      const fill = h('div', {
        class: 'hbar-fill',
        style: { width: `${Math.max(3, Math.round(d.winrate * 100))}%`, background: d.dimmed ? 'var(--muted)' : wrColor(d.winrate) },
      });
      const row = h('div', { class: `hbar-row${d.dimmed ? ' hbar-row--dim' : ''}`, tabindex: '0' }, // K8: Tab-reachable, shows the tooltip on focus
      ...
      tips.attach(row, d.dimmed
        ? `${d.label} · ${d.games} games — below the sample floor`
