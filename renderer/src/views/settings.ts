...
/** Close-to-tray + run-at-login — persisted in the main process. */
function appBehaviorCard(): HTMLElement {
  ...
  function apply(patch: Partial<AppUiSettings>, undoPatch: Partial<AppUiSettings>, label: string): void {
    void bridge.setAppSettings(patch).then((applied) => {
      paint(applied);
      toast(label, {
        action: { label: 'Undo', run: () => void bridge.setAppSettings(undoPatch).then(paint) },
      });
    });
  }
