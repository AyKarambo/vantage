...
- **Row click** opens a drill-down drawer (H7): a role icon next to the hero name, the overall line (games · winrate · W/L — time-share credited, as is the By map list), and a second line stating the filter scope in Players' wording ("all roles · last 30 days · all accounts") so the numbers are never mistaken for the hero's all-time record. Then the per-10 stat grid (KDA, Elims, Deaths, Dmg, Heal, Mit), **By map** (sorted games desc then winrate desc — the common all-1-game case used to have no real tiebreak and read as random order — each map name opening that map's own games via `{ map }` on Matches, `W`-`L` shown instead of a bare game count), and **Recent** (result pill, map, account, date — whole games), where each row is clickable straight into that match's detail page (`HeroDetail.recent[].matchId`, new) instead of being inert text.
