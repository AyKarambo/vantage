...
- **Top priority callouts:** the top 3 net-losing maps (net = losses − wins > 0, from `focusMaps`) with games, signed net, and winrate; hint copy uses practice/review framing; each row is clickable straight into that map's games on Matches (`{ map }`, H3). The CTA **"Open Focus →"** is the Overview's single entry to the Focus screen (Overview teases, Focus is the hub — hierarchy owned by `screen-focus.spec.md`).
