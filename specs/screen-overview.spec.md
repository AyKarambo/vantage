...
- **Maps scatter card:** winrate × volume for every map in range; dot colour is a stable categorical per map; legend uses shortened map names with the full name on hover. Clicking a dot opens that map's own games on Matches (`{ map }`, H3).
