...
- **Coaching card** — the shared editors: the break-reminder editor (on/off + threshold, mechanism owned by `screen-mental.spec.md`), the readiness settings editor (enable + launch-toast, owned by the readiness specs), the target-rotation staleness editor (the thresholds `screen-targets.spec.md`'s rotate nudge uses), and the current-session gap-threshold editor (a preset segmented control, 30m–6h; a hint line states which surfaces the threshold governs — the sidebar's Current session card, the Overview last-session debrief, and Matches' "By sitting" grouping (S4) — and that readiness's own fatigue-curve tuning is separate).
