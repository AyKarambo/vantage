...
export interface MatchPipelineDeps {
   ...
   *  plus the "needs review" holding store (hold on uncertainty / take on resolve / remove on dismiss). */
  history: Pick<HistoryStore, 'add' | 'all' | 'addScreenshots' | 'addPending' | 'takePending' | 'removePending'>;
...
export function createMatchPipeline(deps: MatchPipelineDeps): {
  ...
  function addMatch(record: MatchRecord): void {
    ...
    // then schedule a best-effort end-of-match capture (~2s later, while the
    // summary screen is up). Every failure inside is a logged no-op; a manual
    // log never reaches this branch.
    ...
    // after an account swap), or both. Never silently drop a possibly-real
    // played match: hold it in the SEPARATE pending store so the user can
    // confirm a result (→ history) or dismiss it in Review. It NEVER enters
    // history/analytics until resolved. We do NOT fabricate the game_type.
   ...
   * holding store, stamp the raw outcome (and, when GEP left the game_type
   * unknown, stamp it competitive — the user's confirm is the track decision),
   * and run it back through {@link addMatch} so it lands in history via the
   * identical `matchToGame` → `recordGame` → screenshot path a live capture
   * would. Returns false for an unknown id.
   */
   ...
   * trackable game. Removes it from the holding store WITHOUT logging it (it
   * never touches history/analytics). Returns false for an unknown id. Fires
   * the pending-changed signal only when something was actually removed.
   */
